...
class vbox_controller:
    ...
    def start(self):
        subprocess.call('VBoxManage startvm ' + self.name + ' --type gui', shell = True)
    ...
    def get_cmd_ip(self):
        #first we broadcast a ping
        output = subprocess.check_output('VBoxManage list hostonlyifs | grep IPAddress', shell=True)
        ip = output.split()[1]
        broadcast_ip = ip[:ip.rfind('.')] + '.255'
        print broadcast_ip
        subprocess.call('ping -b -c 2 ' + broadcast_ip, shell=True)
        
        #then we look for the mac in the arp table
        output = subprocess.check_output('VBoxManage showvminfo ' + self.name +  ' | grep "NIC 2"', shell=True).lower() 
        output = output[output.find("mac: ") + len("mac: "):]
        mac = output[:output.find(",")]
        mac_formatted = ''
        print mac
        for i in range(6):
            position = i * 2
    ...
    def stop(self):
        subprocess.call('VBoxManage controlvm ' + self.name + ' poweroff', shell = True)
